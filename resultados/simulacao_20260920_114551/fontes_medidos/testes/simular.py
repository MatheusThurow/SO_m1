...
def executar_cenario(pasta, threads, volume):
    ...
    with (pasta / "console.log").open("w") as console:
        try:
            servidor = subprocess.Popen([str(RAIZ / "servidor.exe"), str(threads)],
                cwd=pasta, stdout=console, stderr=console)
            limite = time.monotonic() + 10
            caminho_log = pasta / "servidor.log"
            while True:
                if caminho_log.exists() and "SERVIDOR PRONTO" in caminho_log.read_text():
                    break
                if servidor.poll() is not None or time.monotonic() > limite:
                    raise RuntimeError("Servidor nao iniciou; consulte console.log")
                time.sleep(0.005)
            # Os processos sao iniciados antes da cronometragem e aguardam stdin.
            for _ in range(CLIENTES + 1):
                clientes.append(subprocess.Popen([str(RAIZ / "cliente.exe")],
                    stdin=subprocess.PIPE, stdout=subprocess.DEVNULL,
                    stderr=subprocess.PIPE, text=True, cwd=pasta))
            cargas = ["".join(f"INSERT {i} Pessoa{i}\n"
                       for i in range(c, volume, CLIENTES)) for c in range(CLIENTES)]
            with ThreadPoolExecutor(max_workers=CLIENTES) as executor:
                inicio = time.perf_counter()
                envios = [executor.submit(cliente.communicate, carga, timeout=120)
            ...
            assert len(ids) == volume and set(ids) == set(range(volume)), "Perda ou duplicacao"
            return {"threads": threads, "requisicoes": volume, "clientes": CLIENTES,
                    "segundos": segundos, "requisicoes_por_segundo": volume / segundos,
                    "corretas": len(ids), "erros": 0,
                    "distribuicao_threads": dict(sorted(distribuicao.items())),
                    "evidencia": str(pasta.name), "status": "aprovado"}
        finally:
...
def main():
    ...
    ambiente = {"data": datetime.now().astimezone().isoformat(),
        "sistema": platform.platform(), "processador": os.environ.get("PROCESSOR_IDENTIFIER"),
        "processadores_logicos": os.cpu_count(), "python": platform.python_version(),
        "compilador": subprocess.check_output(["g++", "--version"], text=True).splitlines()[0],
        "flags": "-std=c++11 -Wall -Wextra -Wpedantic -Iinclude -static (sem -O)",
        "hashes_sha256": {str(p.relative_to(RAIZ)): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in list((RAIZ / "src").glob("*.cpp")) +
            list((RAIZ / "include").glob("*.hpp")) +
            [RAIZ / "cliente.exe", RAIZ / "servidor.exe", Path(__file__)]}}
    dados = {"ambiente": ambiente, "metodo": {"threads": THREADS, "volumes": VOLUMES,
        "clientes": CLIENTES, "repeticoes": REPETICOES, "semente": SEMENTE,
    ...
    funcional = subprocess.run([os.sys.executable, str(RAIZ / "testes" / "testar.py")],
        capture_output=True, text=True, timeout=60)
    (destino / "teste_funcional.txt").write_text(funcional.stdout + funcional.stderr, encoding="utf-8")
    funcional.check_returncode()
    for t in THREADS:
        dados["aquecimentos"].append(executar_cenario(destino / f"aquecimento_t{t}", t, 1000))
        salvar()
        print(f"Aquecimento: {t} threads OK", flush=True)
    ordem = [(t, n, r) for t in THREADS for n in VOLUMES for r in range(1, REPETICOES + 1)]
    random.Random(SEMENTE).shuffle(ordem)
    for indice, (t, n, r) in enumerate(ordem, 1):
        resultado = executar_cenario(destino / f"exec_{indice:02d}_t{t}_n{n}_r{r}", t, n)
        resultado.update({"ordem": indice, "repeticao": r})
        dados["execucoes"].append(resultado)
        salvar()
        print(f"{indice:02d}/{len(ordem)} | threads={t} | n={n} | {resultado['segundos']:.4f}s | OK", flush=True)
